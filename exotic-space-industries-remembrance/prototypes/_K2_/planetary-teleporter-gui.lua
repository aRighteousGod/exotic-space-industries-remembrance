...
local function update_gui(self)
  ...
  local standing_on_teleporter = false

  -- game.print(serpent.block(self.entity.position))
  -- game.print(serpent.block(self.player.position))

  if math.abs(self.entity.position.x - self.player.position.x) < 2 then
    if math.abs(self.entity.position.y - self.player.position.y+1) < 2 then
      standing_on_teleporter = true
    end 
  end 
