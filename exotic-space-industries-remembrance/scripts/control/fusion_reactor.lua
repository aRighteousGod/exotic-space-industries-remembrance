...
function model.entity_check(entity)
  ...
  if entity.name ~= "ei-fusion-reactor" then
      return false
  end
...
function model.open_gui(player)
  if player.gui.relative["ei-fusion-reactor-console"] then
      model.close_gui(player)
  end

  local root = player.gui.relative.add{
      type = "frame",
      name = "ei-fusion-reactor-console",
      anchor = {
...
function model.update_gui(player, data)
      local root = player.gui.relative["ei-fusion-reactor-console"] --[[@as LuaGuiElement]]
...
function model.close_gui(player)
  if player.gui.relative["ei-fusion-reactor-console"] then
      player.gui.relative["ei-fusion-reactor-console"].destroy()
  end
end
