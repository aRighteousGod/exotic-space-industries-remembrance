...
local function set_prerequisites(tech, prerequisite)
    ...
    -- set prerequisites:
    -- if the only current prerequisite is "ei-temp" then simply setting is fine
    -- otherwise add the new prerequisite to the list of prerequisites 
    if data.raw["technology"][tech].prerequisites[1] == "ei-temp" then
...
local function set_prerequisites_for_ages(table_in)
    -- use tech_structure table where keys correspond to "ei-KEY" tech

    for i,v in pairs(table_in) do
        local prerequisite = "ei-"..i
...
local function make_dummy_techs(ages_dummy_dict)
    ...
    for i,v in pairs(data.raw["technology"]) do
        if data.raw["technology"][i].age then
            ...
            local next_dummy_age = ages_dummy_dict[age]
            if not next_dummy_age then
                goto continue
            end

            local next_age = "ei-"..next_dummy_age.."-dummy"
...
-- why is this here?
data.raw.technology["ei-quantum-age"].age = nil
