...
ASSET_NAME = "ei-severance-array-crystal-link"
...
GRAPHICS_DIR = REPO_ROOT / "exotic-space-industries-remembrance" / "graphics" / "entities" / "severance-array"
...
NORMAL_FILENAME = "ei-severance-array-crystal-link.png"
GLOW_FILENAME = "ei-severance-array-crystal-link-glow.png"

# The sheet is positioned in the same frame space as the crystal layer. The
# prototype applies ARRAY_CRYSTAL_LINK_SCALE and ARRAY_CRYSTAL_LINK_SHIFT.
LINK_CENTER_X = 384.0
LINK_TOP_Y = 262.0
LINK_BOTTOM_Y = 438.0
...
def draw_link_frame(index: int, source_frames: list[Image.Image], glow: bool = False) -> Image.Image:
    ...
    rng = random.Random(SEED + index * 193 + (5000 if glow else 0))
    ...
    for y in range(math.floor(LINK_TOP_Y - 18), math.ceil(LINK_BOTTOM_Y + 18)):
        ...
        bend = math.sin(body_t * math.pi * 1.12 + phase * 1.65) * 3.4
        bend += math.sin(body_t * math.tau * 2.4 - phase * 0.75) * 1.6
        center_x = LINK_CENTER_X + bend
        ...
        for x in range(min_x, max_x + 1):
            ...
            sample_x = body_t * BEAM_FRAME_SIZE[0] + index * 5.2
            sample_y = (BEAM_FRAME_SIZE[1] - 1) * 0.5 + lateral * 35.0
            sample_y += math.sin(body_t * 18.0 + phase * 1.8) * 2.1
            ...
            strand = 0.76 + 0.24 * math.sin(y * 0.135 + x * 0.075 + phase * 3.2)
            pulse = 0.70 + 0.30 * math.sin((1.0 - body_t) * math.tau * 3.0 + phase * 2.8)
    ...
    spark_count = 18 if not glow else 12
    for spark_index in range(spark_count):
        lane = (spark_index / max(1, spark_count - 1) + index / FRAME_COUNT * 1.45) % 1.0
        y = LINK_BOTTOM_Y - lane * link_height + rng.uniform(-2.5, 2.5)
        if y < LINK_TOP_Y or y > LINK_BOTTOM_Y:
            continue
        t = (y - LINK_TOP_Y) / link_height
        x = LINK_CENTER_X + math.sin(t * math.pi * 1.4 + phase) * 5.2 + rng.uniform(-9, 9)
        color = spectral_color(rng.uniform(-0.65, 0.65), phase + spark_index * 0.7, 0.75 if glow else 1.0)
        radius = rng.uniform(1.1, 2.6 if not glow else 4.8)
        alpha = rng.randrange(46, 94 if not glow else 82)
...
def make_factorio_preview(link: Image.Image, link_glow: Image.Image) -> Image.Image:
    base = first_frame(GRAPHICS_DIR / "ei-severance-array.png")
    base_shadow = first_frame(GRAPHICS_DIR / "ei-severance-array_base-shadow.png")
    crystal_shadow = first_frame(GRAPHICS_DIR / "ei-severance-array_shadow.png")
    crystal = first_frame(GRAPHICS_DIR / "ei-severance-array_crystal.png")
    ...
    for bg, label in backgrounds:
        ...
        paste_layer(canvas, link, ARRAY_CRYSTAL_SCALE, ARRAY_CRYSTAL_SHIFT)
        paste_layer(canvas, link_glow, ARRAY_CRYSTAL_SCALE, ARRAY_CRYSTAL_SHIFT, glow=True)
...
def write_manifest(specs: list[SheetSpec], previews: list[Path], promoted: bool) -> dict[str, object]:
    ...
    manifest = {
        ...
        "mode": "severance-array-crystal-link",
        "source_assets": [
            str((BEAM_DIR / "ei-severance-array-beam-body.png").relative_to(REPO_ROOT)).replace("\\", "/"),
            str((BEAM_DIR / "ei-severance-array-beam-body-glow.png").relative_to(REPO_ROOT)).replace("\\", "/"),
        ],
        ...
        "frame": {
            ...
            "shift": list(ARRAY_CRYSTAL_SHIFT),
...
def generate(promote: bool) -> dict[str, object]:
    ...
    body_path = BEAM_DIR / "ei-severance-array-beam-body.png"
    glow_path = BEAM_DIR / "ei-severance-array-beam-body-glow.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate the Severance Array chromatic crystal-link overlay sheets.")
    parser.add_argument("--promote", action="store_true", help="Copy generated PNGs into the live Severance Array crystal-link graphics folder.")
