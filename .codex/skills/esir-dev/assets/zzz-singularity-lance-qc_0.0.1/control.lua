local REPORT_PATH = "singularity-lance-qc.jsonl"
local HELPER_REMOTE = "zzz-singularity-lance-qc"
local MAIN_QC_REMOTE = "exotic-industries-qc"
local FORCE_NAME = "singularity-lance-qc"
local SURFACE_NAME = "singularity-lance-qc"
...
local function ensure_state()
  storage.singularity_lance_qc = storage.singularity_lance_qc or {}
  local state = storage.singularity_lance_qc
...
local function write_record(record)
  ...
  log("SINGULARITY_LANCE_QC " .. encoded)
...
local function get_remote_status()
  ...
  return {
    ...
    has_reset = interface and interface.reset_singularity_lance_runtime ~= nil or false,
    has_configure = interface and interface.configure_singularity_lance_qc ~= nil or false,
    has_service = interface and interface.service_singularity_lance_qc ~= nil or false,
    has_snapshot = interface and interface.get_singularity_lance_qc_snapshot ~= nil or false,
  }
end
...
local function extract_singularity_lance_timing(snapshot)
  ...
  local timing = table_child(snapshot, "singularity_lance")
    or table_child(snapshot, "timings")
    or table_child(snapshot, "timing")
  ...
  return {
    ...
    p95_ms = first_number(
      ...
      table_child(timing, "p95_ms"),
      table_child(timing, "p95"),
      table_child(timing, "p95_update_ms")
    ),
    max_ms = first_number(
      table_child(snapshot, "max_ms"),
      table_child(snapshot, "max_update_ms"),
      table_child(timing, "max_ms"),
      table_child(timing, "max"),
      table_child(timing, "max_update_ms")
    ),
    average_ms = first_number(
      table_child(snapshot, "average_ms"),
      table_child(snapshot, "average_update_ms"),
      table_child(timing, "average_ms"),
      table_child(timing, "average_update_ms")
    ),
    last_ms = first_number(
      table_child(snapshot, "last_ms"),
      table_child(snapshot, "last_update_ms"),
      table_child(timing, "last_ms"),
      table_child(timing, "last_update_ms")
    ),
    sample_count = first_number(
      table_child(snapshot, "sample_count"),
      table_child(timing, "sample_count"),
      table_child(timing, "samples"),
      table_child(timing, "count")
    ),
  }
end
...
local function sync_main_runtime(reason, current_tick)
  local reset_ok, reset_result = call_main_qc("reset_singularity_lance_runtime", reason or "singularity-lance-qc", current_tick or game.tick)
  local configure_ok, configure_result = call_main_qc("configure_singularity_lance_qc", {
    reset = true,
    qc_enabled = true,
    profiling_enabled = true,
  })
  local service_ok, service_result = call_main_qc("service_singularity_lance_qc", SERVICE_LIMIT)
...
local function collect_main_snapshot()
  local ok, snapshot = call_main_qc("get_singularity_lance_qc_snapshot")
  if ok and type(snapshot) == "table" then
    return {
      ok = true,
      snapshot = snapshot,
      timing = extract_singularity_lance_timing(snapshot),
    }
  end
  ...
  return {
    ok = false,
    error = snapshot,
    timing = extract_singularity_lance_timing(nil),
  }
end
...
local function run_due_checkpoints(current_tick)
  ...
  while checkpoint and current_tick >= state.base_tick + checkpoint do
    call_main_qc("service_singularity_lance_qc", SERVICE_LIMIT)
